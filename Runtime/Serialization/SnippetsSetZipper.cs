...
namespace Snippets.Sdk
{
    ...
    public class SnippetsSetZipper : ISnippetsSetZipper
    {
        ...
        private SnippetsSetDataDto DeepCopyDto(SnippetsSetDataDto original)
        {
            ...
            for (int i = 0; i < original.Snippets.Length; i++)
            {
                copySnippetsSet.Snippets[i] = new SnippetDataDto
                {
                    ...
                    AudioTimestamps = new SnippetAudioTimestampsData(original.Snippets[i].AudioTimestamps)
                };
            }
        ...
        private SnippetsSetDataDto GetDtoFromServerJsonMetadata(string serverJsonMetadata)
        {
            ...
            for (int i = 0; i < serverSnippetsSetDto.snippets.Length; i++)
            {
                snippetsSetDataDto.Snippets[i] = new SnippetDataDto
                {
                    ...
                    AudioTimestamps = CreateAudioTimestamps(serverSnippetsSetDto.snippets[i].audio_timestamps)
                };
            }
        ...
        private static SnippetAudioTimestampsData CreateAudioTimestamps(ServerAudioTimestampsDto audioTimestamps)
        {
            if (audioTimestamps == null)
            {
                return new SnippetAudioTimestampsData();
            }

            return new SnippetAudioTimestampsData
            {
                Characters = audioTimestamps.characters != null ? new System.Collections.Generic.List<string>(audioTimestamps.characters) : new System.Collections.Generic.List<string>(),
                CharacterStartTimes = audioTimestamps.character_start_times_seconds != null ? new System.Collections.Generic.List<float>(audioTimestamps.character_start_times_seconds) : new System.Collections.Generic.List<float>(),
                CharacterEndTimes = audioTimestamps.character_end_times_seconds != null ? new System.Collections.Generic.List<float>(audioTimestamps.character_end_times_seconds) : new System.Collections.Generic.List<float>()
            };
        }
        ...
        public class ServerSnippetDataDto
        {
            ...
            public ServerAudioTimestampsDto audio_timestamps;
        }

        [Serializable]
        public class ServerAudioTimestampsDto
        {
