...
namespace Snippets.Sdk
{
    ...
    public class BackendLoginManager : ILoginManager, IDisposable
    {
        ...
        /// <summary>
        /// The PlayerPrefs key for storing the username for the current backend environment.
        /// </summary>
        private string UsernameKey => $"SnippetsSDK.{GetCredentialScope()}.U";

        /// <summary>
        /// The PlayerPrefs key for storing the refresh token for the current backend environment.
        /// </summary>
        private string TokenKey => $"SnippetsSDK.{GetCredentialScope()}.T";
        ...
        /// <summary>
        /// Builds a stable storage scope so credentials do not leak across production and staging.
        /// </summary>
        private string GetCredentialScope()
        {
            if (m_backendApiCaller is BackendApiCaller backendApiCaller &&
                !string.IsNullOrWhiteSpace(backendApiCaller.BaseUrl))
            {
                return SanitizePlayerPrefsKeyComponent(backendApiCaller.BaseUrl);
            }

            return "default";
        }

        private static string SanitizePlayerPrefsKeyComponent(string value)
        {
            char[] buffer = new char[value.Length];

            for (int i = 0; i < value.Length; i++)
            {
                buffer[i] = char.IsLetterOrDigit(value[i]) ? value[i] : '_';
            }
