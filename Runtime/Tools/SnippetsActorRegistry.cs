...
public class SnippetsActorRegistry : MonoBehaviour
{
    ...
    public enum CustomAnimationPlaybackMode
    {
        [InspectorName("Play Once Then Idle")]
        PlayOnceThenIdle,
        [InspectorName("Loop Until Snippet Ends")]
        LoopUntilSnippetEnds
    }
    ...
    class Runtime
    {
        ...
        public UnityAction customAnimationSnippetCleanup;
        public Coroutine customAnimationCompletionCoroutine;
    }
    ...
    void OnDisable()
    {
        ...
        CancelAllCustomAnimationLifecycles();
    ...
    void StopSnippetPlaybackAndAudio(Actor actor)
    {
        ...
        CancelCustomAnimationLifecycle(actor);
    ...
    void FadeToLoopNow(int actorIndex, bool isWalk)
    {
        ...
        CancelCustomAnimationLifecycle(actor, rt);
    ...
    public void PlayCustomAnimationOnce(int actorIndex, int customAnimationIndex, bool stopSnippetPlayback = true)
    {
        ...
        ScheduleIdleAfterCustomAnimation(actorIndex);
    }
    ...
    public void StartSnippetSpeechWithCustomAnimationNow(
        ...
        CustomAnimationPlaybackMode playbackMode = CustomAnimationPlaybackMode.PlayOnceThenIdle)
    {
        var actor = GetActor(actorIndex);
        if (actor == null || actor.player == null || actor.legacyAnimation == null) return;
        if (!_rt.TryGetValue(actor, out var rt)) return;
        ...
        CrossFadeOrPlay(
            ...
            rt,
            customClip,
            playbackMode == CustomAnimationPlaybackMode.LoopUntilSnippetEnds
                ? WrapMode.Loop
                : WrapMode.Once);
        ...
        if (playbackMode == CustomAnimationPlaybackMode.LoopUntilSnippetEnds)
            AttachCustomAnimationSnippetCleanup(actor, rt);

        actor.player.Play();
    }

    public void PlaySnippetSpeechWithCustomAnimation(
        int actorIndex,
        int snippetIndex,
        int customAnimationIndex,
        SnippetMaskMode snippetMaskMode = SnippetMaskMode.None,
        CustomAnimationPlaybackMode playbackMode = CustomAnimationPlaybackMode.PlayOnceThenIdle)
    {
        StartSnippetSpeechWithCustomAnimationNow(
            actorIndex,
            snippetIndex,
        ...
        if (playbackMode == CustomAnimationPlaybackMode.PlayOnceThenIdle)
            ScheduleIdleAfterCustomAnimation(actorIndex);
    }
    ...
    public void PlaySnippetSpeechWithCustomAnimationOnce(
        ...
        SnippetMaskMode snippetMaskMode = SnippetMaskMode.None)
    {
        PlaySnippetSpeechWithCustomAnimation(
            actorIndex,
            snippetIndex,
            customAnimationIndex,
            snippetMaskMode,
            CustomAnimationPlaybackMode.PlayOnceThenIdle);
    }
    ...
    void CrossFadeOrPlay(Actor actor, Runtime rt, AnimationClip clip, WrapMode wrapMode = WrapMode.Once)
    {
        if (actor == null || actor.legacyAnimation == null || clip == null) return;
    ...
    void ScheduleIdleAfterCustomAnimation(int actorIndex)
    {
        var actor = GetActor(actorIndex);
        ...
        if (!_rt.TryGetValue(actor, out var rt))
            return;
        if (string.IsNullOrEmpty(rt.activeCustomAlias))
            return;

        CancelCustomAnimationCompletion(rt);
        rt.customAnimationCompletionCoroutine = StartCoroutine(
            WaitForCustomAnimationAndReturnToIdle(actor, rt, rt.activeCustomAlias));
    }

    IEnumerator WaitForCustomAnimationAndReturnToIdle(Actor actor, Runtime rt, string alias)
    {
        yield return null;

        while (actor != null &&
               actor.legacyAnimation != null &&
               string.Equals(rt.activeCustomAlias, alias, StringComparison.Ordinal) &&
               actor.legacyAnimation.IsPlaying(alias))
        {
            yield return null;
        }

        rt.customAnimationCompletionCoroutine = null;
        ...
        int actorIndex = actors != null ? actors.IndexOf(actor) : -1;
        if (actorIndex >= 0 && actor.idleClip != null)
            FadeToIdleNow(actorIndex);
        else
            ScheduleBlendshapeNeutralization(actor, 0f);
    }

    void CancelCustomAnimationCompletion(Runtime rt)
    {
        if (rt?.customAnimationCompletionCoroutine == null)
            return;

        StopCoroutine(rt.customAnimationCompletionCoroutine);
        rt.customAnimationCompletionCoroutine = null;
    }

    void AttachCustomAnimationSnippetCleanup(Actor actor, Runtime rt)
    {
        if (actor?.player == null || rt == null)
            return;

        DetachCustomAnimationSnippetCleanup(actor, rt);

        rt.customAnimationSnippetCleanup = () => CompleteLoopingCustomAnimation(actor);
        actor.player.PlaybackStopped.AddListener(rt.customAnimationSnippetCleanup);
    }

    void DetachCustomAnimationSnippetCleanup(Actor actor, Runtime rt)
    {
        if (actor?.player == null || rt?.customAnimationSnippetCleanup == null)
            return;

        actor.player.PlaybackStopped.RemoveListener(rt.customAnimationSnippetCleanup);
        rt.customAnimationSnippetCleanup = null;
    }

    void CancelCustomAnimationLifecycle(Actor actor)
    {
        if (actor == null || !_rt.TryGetValue(actor, out var rt))
            return;

        CancelCustomAnimationLifecycle(actor, rt);
    }

    void CancelCustomAnimationLifecycle(Actor actor, Runtime rt)
    {
        CancelCustomAnimationCompletion(rt);
        DetachCustomAnimationSnippetCleanup(actor, rt);
    }

    void CancelAllCustomAnimationLifecycles()
    {
        foreach (var entry in _rt)
            CancelCustomAnimationLifecycle(entry.Key, entry.Value);
    }

    void CompleteLoopingCustomAnimation(Actor actor)
    {
        if (actor == null || !_rt.TryGetValue(actor, out var rt))
            return;
        ...
        {
            if (!string.IsNullOrEmpty(rt.activeCustomAlias) && actor.legacyAnimation != null)
                actor.legacyAnimation.Stop(rt.activeCustomAlias);
