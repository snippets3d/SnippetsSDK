...
public class SnippetsHeadTurn : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        // Capture animated baselines ONCE (legacy Animation has written by now if your order is correct)
        ...
        // Virtual driver pivot world position (same as driver version)
        Vector3 driverWorldPos = _headParent.TransformPoint(_headDriverRestLocalPos);
        ...
        Quaternion desiredHeadWorld = Quaternion.LookRotation(worldDir.normalized, Vector3.up);

        // Solve against the virtual driver world (matches driver-script behavior)
        Quaternion currentDriverWorld = _headParent.rotation * _headDriverLocalRot;

        // deltaWorld rotates current driver world into desired head world
        Quaternion deltaWorld = desiredHeadWorld * Quaternion.Inverse(currentDriverWorld);

        Quaternion desiredDriverWorld = deltaWorld * currentDriverWorld; // == desiredHeadWorld
        Quaternion desiredDriverLocal = Quaternion.Inverse(_headParent.rotation) * desiredDriverWorld;

        // Clamp relative to rest (same as accurate script)
        Quaternion rel = Quaternion.Inverse(_headDriverRestLocalRot) * desiredDriverLocal;
        Vector3 relEuler = ToSignedEuler(rel.eulerAngles);

        float clampedYaw = Mathf.Clamp(relEuler.y, -maxYaw, maxYaw);
        float clampedPitch = Mathf.Clamp(relEuler.x, -maxPitch, maxPitch);

        Quaternion clampedLocal = _headDriverRestLocalRot * Quaternion.Euler(clampedPitch, clampedYaw, 0f);
        ...
        UpdateWaistFromAimPoint(animWaistWorld, aimPoint, clampedYaw, hasTarget: true);
    }
