...
namespace Snippets.Sdk
{
    ...
    public class BackendApiCaller : IBackendApiCaller
    {
        ...
        /// <summary>
        /// Gets the base URL currently used for relative endpoints.
        /// </summary>
            ...
            TBodyValueType parameter, string authToken = null, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                using (UnityWebRequest www = UnityWebRequest.Post(fullEndpoint, jsonData, "application/json"))
                {
                    ...
                    {
                        return new BackendApiResponse<TReturnValueType>(default(TReturnValueType), ApiResponseCode.Error, www.responseCode, BuildErrorMessage(www, fullEndpoint));
                    }
                }
            }
            ...
            try
            {
                ...
                using (UnityWebRequest www = UnityWebRequest.Get(fullEndpoint))
                {
                    ...
                    else
                    {
                        ...
                        return new BackendApiResponse(ApiResponseCode.Error, www.responseCode, BuildErrorMessage(www, fullEndpoint));
                    }
                }
            }
            // The request threw a web exception
        ...
        private static string BuildErrorMessage(UnityWebRequest request, string fullEndpoint)
        {
            var error = string.IsNullOrWhiteSpace(request.error) ? "Request failed" : request.error;
            ...
            if (!string.IsNullOrWhiteSpace(responseBody))
            {
                ...
                return $"{error} | Endpoint: {fullEndpoint} | Response: {responseBody}";
            }

            return $"{error} | Endpoint: {fullEndpoint}";
        }
    }
}
