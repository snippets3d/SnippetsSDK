...
namespace Snippets.Sdk
{
        ...
        ISnippetSetsProvider
    {
        ...
        public async Task<ApiResponse<SnippetsSetMetadata[]>> GetAllSnippetsSets(CancellationToken cancellationToken = default)
        {
            ...
            var snippetsReponse = await BackendApiCaller.JsonRestGetRequest<GetSnippetsSetsResponseData>("/avatar/snippetsets/?offset=0&limit=100", LoginManager.CurrentToken, cancellationToken);

            // if the call was successful, convert the data to the expected format
            if (snippetsReponse.IsSuccessful)
            {
                var snippetSets = snippetsReponse.Value.snippetSets ?? Array.Empty<ResponseSnippetsSetData>();
                var orderedSnippetSets = snippetSets
                    .OrderByDescending(s => s.updated_at)
                    .ToArray();

                var snippetsSetsMetadataDto = new SnippetsSetMetadataDto[orderedSnippetSets.Length];
                var anyThumbnailsDownloaded = false;

                for(int i = 0; i < snippetsSetsMetadataDto.Length; i++)
                {
                    var snippetSetData = orderedSnippetSets[i];
                    ...
                    var downloaded = await DownloadThumbnail(snippetsSetsMetadataDto[i], cancellationToken);
                    anyThumbnailsDownloaded |= downloaded;
                }

                //force an asset reload only if we downloaded new thumbnails,
                //so the Unity editor will import the new files
                if (anyThumbnailsDownloaded)
                {
                    AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
                }

                //ensure thumbnails are imported as Sprites (avoid reimport if already correct)
                foreach (var snippetSet in snippetsSetsMetadataDto)
                {
                    if (!string.IsNullOrEmpty(snippetSet.ThumbnailPath))
                    {
                        AssetDatabase.ImportAsset(snippetSet.ThumbnailPath, ImportAssetOptions.ForceSynchronousImport);
                        AssetsUtilities.SetTextureImporterSettingsIfNeeded(snippetSet.ThumbnailPath, TextureImporterType.Sprite);
                    }
                }
        ...
        /// <returns></returns>
        private async Task<bool> DownloadThumbnail(SnippetsSetMetadataDto snippetsSet, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var thumbnailUri = new Uri(snippetsSet.ThumbnailPath, UriKind.Absolute);

                // get where to locally download the thumbnail. We'll put them under the generated snippets root, in a dedicated subfolder
                var thumbnailFileName = "thumbnail.png";
                var snippetSetFolderName = string.IsNullOrWhiteSpace(snippetsSet.Id) ? "snippetset" : snippetsSet.Id;
                var thumbnailsRootFolder = GetThumbnailsRootFolder();
                var thumbnailLocalPath = IoUtilities.GetProjectRelativePath(Path.Combine(thumbnailsRootFolder, SnippetsThumbnailDirectoryName, snippetSetFolderName, thumbnailFileName));
        ...
        private string GetThumbnailsRootFolder()
        {
            if (m_projectSnippetsSettings != null)
            {
                if (!string.IsNullOrWhiteSpace(m_projectSnippetsSettings.GeneratedSnippetsDownloadFolder))
                {
                    return m_projectSnippetsSettings.GeneratedSnippetsDownloadFolder;
                }

                if (!string.IsNullOrWhiteSpace(m_projectSnippetsSettings.RawSnippetsDownloadFolder))
                {
                    return m_projectSnippetsSettings.RawSnippetsDownloadFolder;
                }
            }

            return "Assets/My Snippets";
        }
        ...
        private SnippetsSetMetadataDto ConvertResponseDataToMetadataDto(ResponseSnippetsSetData snippetSetData)
        {
            return new SnippetsSetMetadataDto()
            {
                ...
                ThumbnailPath = snippetSetData.thumbnail_url,
