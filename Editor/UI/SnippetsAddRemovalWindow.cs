...
namespace Snippets.Sdk.Ui
{
    ...
    public abstract class SnippetsAddRemovalWindow : EditorWindow
    {
        ...
        private async void Update()
        {
            ...
            {
                var loginResponse = await LoginManager.LoginWithLastValidCredentials();
                if (!loginResponse.IsSuccessful || !LoginManager.IsLoggedIn)
                {
        ...
        protected virtual void UpdateUI()
        {
            ...
            string rawFolder = SnippetsSetsFolderFacilities.GetSnippetsSetsRawFolder(m_projectSettings, ToOperateSnippetsSetMetadata);
            string generatedFolder = SnippetsSetsFolderFacilities.GetSnippetsSetsGeneratedFolder(m_projectSettings, ToOperateSnippetsSetMetadata);

            // Display paths using forward slashes for consistency in the UI.
            rootVisualElement.Q<Label>("raw-directory-label").text = rawFolder.Replace('\\', '/');
            rootVisualElement.Q<Label>("generated-directory-label").text = generatedFolder.Replace('\\', '/');
        }
